...
public class IconResource {
    ...
    private static void loadDefaultIcon() {
        try {
            ...
            loadImageFromJarFile();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static void loadImageFromLocalFile() throws Exception {
        ClassLoader classLoader = IconResource.class.getClassLoader();
        URL url = classLoader.getResource("images/icons");
        if (url == null) {
            return;
        }
        ...
        if (files != null) {
            for (File f : files) {
                ...
                ICONS.put(iconName, new ImageIcon(f.getAbsolutePath()));
            }
        }
    }

    private static void loadImageFromJarFile() throws Exception {
        ClassLoader classLoader = IconResource.class.getClassLoader();
